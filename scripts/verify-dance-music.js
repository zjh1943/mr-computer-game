...
(async()=>{
 ...
 const scope={setTimeout:(f,ms)=>{timers.set(++id,{f,ms});return id},clearTimeout:i=>timers.delete(i),document:{createElement:()=>({})},location:{origin:'http://localhost'},window:{DanceCast:require('../dance-cast'),DanceReferenceData:require('../dance-reference-data'),YT:{Player:class{constructor(h,o){this.options=o;players.push(this)}destroy(){destroyed++}playVideo(){plays++}unMute(){}getCurrentTime(){return 12}}}}};
 ...
 const control=await scope.window.DanceMusic.open(host,{video:'test-source'},events);
 ...
 host.isConnected=false;assert.equal(await scope.window.DanceMusic.open(host,{video:'test-source'},events),null);
