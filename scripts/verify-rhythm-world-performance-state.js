...
(async () => {
  ...
  const duetPulse = app.match(/function duetPulse\(\)\{[^}]+\}/)?.[0] || '';
  assert(!duetPulse.includes('speakPlayer'), 'singing must animate without a blocking player speech bubble');
  assert(!duetPulse.includes('speakNpc'), 'singing must animate without blocking NPC speech bubbles');
